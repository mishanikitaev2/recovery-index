...
BASE_SEED_FILE = ROOT_DIR / "data" / "companies_seed.csv"
RAW_RUN_DIR = ROOT_DIR / "service" / "raw"
DATA_DIR = ROOT_DIR / "data"
OUTPUT_FILE = DATA_DIR / "model_dataset.csv"
FNS_SSCH_DIR = DATA_DIR / "fns_ssch"
FNS_RSMP_DIR = DATA_DIR / "fns_rsmp"
OBSERVATION_END = pd.Timestamp("2026-04-23")
MIN_FAILED_OUTCOME_DATE = pd.Timestamp("2022-01-01")
MAX_FAILED_OUTCOME_DATE = OBSERVATION_END
FORECAST_HORIZON_MONTHS = 12
COURT_LOOKBACK_YEARS = 2
REQUIRE_DEFENDANT_STRESS = True
BURST_MAX_EMPTY_MONTHS = 1
CLAIM_RATIO_MONEY_FLOOR = 100_000.0
...
def signed_log_ratio(numerator: Any, denominator: Any, floor: float) -> float:
    left = max(safe_float(numerator) or 0.0, 0.0)
    right = max(safe_float(denominator) or 0.0, 0.0)
    return math.log((left + floor) / (right + floor))

def signed_fold_ratio(numerator: Any, denominator: Any, floor: float) -> float:
    left = max(safe_float(numerator) or 0.0, 0.0) + floor
    right = max(safe_float(denominator) or 0.0, 0.0) + floor
    if left >= right:
        return (left / right) - 1.0
    return -((right / left) - 1.0)

_FNS_HEADCOUNT_CACHE: dict[int, dict[str, float]] = {}
_CHECKO_STAFF_HISTORY_CACHE: dict[str, dict[int, float]] | None = None
_FNS_RSMP_CACHE: pd.DataFrame | None = None

def fns_headcount_for_year(company_inn: str, year: int) -> float | None:
    if year not in _FNS_HEADCOUNT_CACHE:
        path = FNS_SSCH_DIR / f"headcount_{year}.csv"
        if not path.exists():
            _FNS_HEADCOUNT_CACHE[year] = {}
        else:
            frame = pd.read_csv(path, dtype={"company_inn": "string"})
            _FNS_HEADCOUNT_CACHE[year] = {
                str(row.company_inn): float(row.profile_headcount)
                for row in frame.itertuples(index=False)
                if pd.notna(row.profile_headcount)
            }
    return _FNS_HEADCOUNT_CACHE[year].get(str(company_inn))

def checko_headcount_for_year(company_inn: str, year: int) -> float | None:
    global _CHECKO_STAFF_HISTORY_CACHE
    if _CHECKO_STAFF_HISTORY_CACHE is None:
        path = DATA_DIR / "checko_staff_history.csv"
        cache: dict[str, dict[int, float]] = {}
        if path.exists():
            frame = pd.read_csv(path, dtype={"company_inn": "string"}, low_memory=False)
            year_columns = [column for column in frame.columns if column.startswith("headcount_")]
            for row in frame.itertuples(index=False):
                inn = str(getattr(row, "company_inn"))
                history: dict[int, float] = {}
                for column in year_columns:
                    value = getattr(row, column)
                    if pd.notna(value):
                        history[int(column.removeprefix("headcount_"))] = float(value)
                if history:
                    cache[inn] = history
        _CHECKO_STAFF_HISTORY_CACHE = cache
    return _CHECKO_STAFF_HISTORY_CACHE.get(str(company_inn), {}).get(year)

def fns_rsmp_asof(company_inn: str, assessment_date: pd.Timestamp) -> pd.Series | None:
    global _FNS_RSMP_CACHE
    if _FNS_RSMP_CACHE is None:
        frames = []
        for path in sorted(FNS_RSMP_DIR.glob("rsmp_*.csv")):
            frame = pd.read_csv(path, dtype={"company_inn": "string"}, low_memory=False)
            if frame.empty:
                continue
            frame["rsmp_release_date"] = pd.to_datetime(frame["rsmp_release_date"], errors="coerce")
            frame["profile_headcount"] = pd.to_numeric(frame.get("profile_headcount"), errors="coerce")
            frame["profile_headcount_source_year"] = pd.to_numeric(
                frame.get("profile_headcount_source_year"),
                errors="coerce",
            )
            frames.append(frame)
        if frames:
            combined = pd.concat(frames, ignore_index=True)
            combined = combined.dropna(subset=["company_inn", "rsmp_release_date"])
            _FNS_RSMP_CACHE = combined.sort_values(["company_inn", "rsmp_release_date"]).reset_index(drop=True)
        else:
            _FNS_RSMP_CACHE = pd.DataFrame()
    if _FNS_RSMP_CACHE.empty or pd.isna(assessment_date):
        return None
    rows = _FNS_RSMP_CACHE[
        _FNS_RSMP_CACHE["company_inn"].eq(str(company_inn))
        & _FNS_RSMP_CACHE["rsmp_release_date"].le(pd.Timestamp(assessment_date))
    ]
    if rows.empty:
        return None
    return rows.iloc[-1]
...
def build_case_frame(
    company_inn: str,
    min_case_date: pd.Timestamp | None = None,
    max_case_date: pd.Timestamp | None = None,
) -> pd.DataFrame:
    ...
    for page in pages:
        ...
        for item in items:
            ...
            if pd.notna(case_date):
                if min_case_date is not None and case_date < min_case_date:
                    continue
                if max_case_date is not None and case_date > max_case_date:
                    continue
            ...
            plaintiffs = item.get("Ист") or item.get("Истцы") or []
            defendants = item.get("Ответ") or item.get("Ответчики") or []
            ...
            rows.append(
                {
                    ...
                    "role": role,
                    "is_defendant": int(role in ("defendant", "both")),
                    "is_plaintiff": int(role in ("plaintiff", "both")),
                    "case_one": 1,
                }
            )
    if not rows:
...
def aggregate_case_window(case_frame: pd.DataFrame, anchor_date: pd.Timestamp, months: int, prefix: str) -> dict[str, Any]:
    ...
    if case_frame.empty or pd.isna(anchor_date):
        for field in (
            ...
            "plaintiff_active_months",
        ):
            output[f"{prefix}_{field}"] = 0.0
        output |= court_pressure_features(prefix, 0.0, 0.0)
        return output
    ...
    defendant_mask = subset["is_defendant"].astype(bool)
    plaintiff_mask = subset["is_plaintiff"].astype(bool)
    ...
    output[f"{prefix}_defendant_count"] = float(defendant_mask.sum())
    output[f"{prefix}_plaintiff_count"] = float(plaintiff_mask.sum())
    output[f"{prefix}_defendant_claim_sum"] = float(subset.loc[defendant_mask, "claim_amount"].sum())
    output[f"{prefix}_defendant_claim_max"] = (
        float(subset.loc[defendant_mask, "claim_amount"].max()) if defendant_mask.any() else 0.0
    )
    output[f"{prefix}_plaintiff_claim_sum"] = float(subset.loc[plaintiff_mask, "claim_amount"].sum())
    output[f"{prefix}_plaintiff_claim_max"] = (
        float(subset.loc[plaintiff_mask, "claim_amount"].max()) if plaintiff_mask.any() else 0.0
    )
    output[f"{prefix}_active_months"] = float(subset["case_month"].nunique()) if not subset.empty else 0.0
    output[f"{prefix}_defendant_active_months"] = (
        float(subset.loc[defendant_mask, "case_month"].nunique()) if defendant_mask.any() else 0.0
    )
    output[f"{prefix}_plaintiff_active_months"] = (
        float(subset.loc[plaintiff_mask, "case_month"].nunique()) if plaintiff_mask.any() else 0.0
    )
    output |= court_pressure_features(prefix, output[f"{prefix}_defendant_claim_sum"], output[f"{prefix}_plaintiff_claim_sum"])
    return output
...
def court_pressure_features(prefix: str, defendant_claim_sum: Any, plaintiff_claim_sum: Any) -> dict[str, float]:
    defendant = max(safe_float(defendant_claim_sum) or 0.0, 0.0)
    plaintiff = max(safe_float(plaintiff_claim_sum) or 0.0, 0.0)
    return {
        f"{prefix}_signed_claim_pressure": float(np.log1p(defendant) - np.log1p(plaintiff)),
        f"{prefix}_plaintiff_offset_share": float(plaintiff / (defendant + plaintiff + 1.0)),
    }
...
def build_bursts(case_frame: pd.DataFrame) -> list[dict[str, Any]]:
    if case_frame.empty:
        return []
    work = case_frame.copy()
    defendant_mask = work["is_defendant"].astype(bool)
    plaintiff_mask = work["is_plaintiff"].astype(bool)
    work["defendant_claim_amount"] = np.where(defendant_mask, work["claim_amount"], 0.0)
    work["plaintiff_claim_amount"] = np.where(plaintiff_mask, work["claim_amount"], 0.0)
    monthly = (
        work.groupby("case_month", as_index=False)
        .agg(
            total_cases=("case_one", "sum"),
            claim_sum=("claim_amount", "sum"),
            claim_max=("claim_amount", "max"),
            defendant_cases=("is_defendant", "sum"),
            plaintiff_cases=("is_plaintiff", "sum"),
            defendant_claim_sum=("defendant_claim_amount", "sum"),
            defendant_claim_max=("defendant_claim_amount", "max"),
            plaintiff_claim_sum=("plaintiff_claim_amount", "sum"),
            plaintiff_claim_max=("plaintiff_claim_amount", "max"),
        )
        .sort_values("case_month")
        .reset_index(drop=True)
    )
    monthly = monthly[monthly["defendant_cases"] > 0].reset_index(drop=True)
    if monthly.empty:
        return []
    ...
    for row in monthly.to_dict("records"):
        ...
        if month_diff(prev, month) <= BURST_MAX_EMPTY_MONTHS + 1:
            current.append(row)
        else:
    ...
    for chunk in bursts:
        ...
        expected_cases = prev_case_mean * burst_months
        expected_claim_sum = prev_claim_mean * burst_months
        ...
        defendant_peak_cases = float(max(item["defendant_cases"] for item in chunk))
        plaintiff_peak_cases = float(max(item["plaintiff_cases"] for item in chunk))
        defendant_peak_claim_sum = float(max(item["defendant_claim_sum"] for item in chunk))
        plaintiff_peak_claim_sum = float(max(item["plaintiff_claim_sum"] for item in chunk))
        defendant_peak_claim_max = float(max(item["defendant_claim_max"] for item in chunk))
        plaintiff_peak_claim_max = float(max(item["plaintiff_claim_max"] for item in chunk))
        defendant_last_month_cases = float(chunk[-1]["defendant_cases"])
        plaintiff_last_month_cases = float(chunk[-1]["plaintiff_cases"])
        defendant_last_month_claim_sum = float(chunk[-1]["defendant_claim_sum"])
        plaintiff_last_month_claim_sum = float(chunk[-1]["plaintiff_claim_sum"])
        ...
        output.append(
            {
                ...
                "expected_claim_sum": expected_claim_sum,
                "case_ratio": total_cases / max(expected_cases, 1.0),
                "claim_ratio": total_claim_sum / max(expected_claim_sum, 1.0),
                "case_pressure_log_signed": signed_log_ratio(total_cases, expected_cases, 1.0),
                "claim_pressure_log_signed": signed_log_ratio(total_claim_sum, expected_claim_sum, CLAIM_RATIO_MONEY_FLOOR),
                "case_pressure_signed_ratio": signed_fold_ratio(total_cases, expected_cases, 1.0),
                "claim_pressure_signed_ratio": signed_fold_ratio(total_claim_sum, expected_claim_sum, CLAIM_RATIO_MONEY_FLOOR),
                "case_baseline_tiny_flag": int(expected_cases < 1.0),
                "claim_baseline_tiny_flag": int(expected_claim_sum < CLAIM_RATIO_MONEY_FLOOR),
                ...
                "defendant_claim_max": defendant_peak_claim_max,
                "defendant_peak_cases": defendant_peak_cases,
                "defendant_peak_claim_sum": defendant_peak_claim_sum,
                "defendant_peak_claim_max": defendant_peak_claim_max,
                "defendant_last_month_cases": defendant_last_month_cases,
                "defendant_last_month_claim_sum": defendant_last_month_claim_sum,
                "defendant_cases_per_month": defendant_cases / max(burst_months, 1.0),
                "defendant_claim_per_month": defendant_claim_sum / max(burst_months, 1.0),
                "defendant_avg_claim_per_case": defendant_claim_sum / max(defendant_cases, 1.0),
                "defendant_peak_claim_share": defendant_peak_claim_max / max(defendant_claim_sum, 1.0),
                "plaintiff_claim_sum": plaintiff_claim_sum,
                "plaintiff_claim_max": plaintiff_peak_claim_max,
                "plaintiff_peak_cases": plaintiff_peak_cases,
                "plaintiff_peak_claim_sum": plaintiff_peak_claim_sum,
                "plaintiff_peak_claim_max": plaintiff_peak_claim_max,
                "plaintiff_last_month_cases": plaintiff_last_month_cases,
                "plaintiff_last_month_claim_sum": plaintiff_last_month_claim_sum,
                "plaintiff_cases_per_month": plaintiff_cases / max(burst_months, 1.0),
                "plaintiff_claim_per_month": plaintiff_claim_sum / max(burst_months, 1.0),
                "plaintiff_avg_claim_per_case": plaintiff_claim_sum / max(plaintiff_cases, 1.0),
                "plaintiff_peak_claim_share": plaintiff_peak_claim_max / max(plaintiff_claim_sum, 1.0),
            }
        )
    return output
...
def burst_features(burst: dict[str, Any] | None, prefix: str) -> dict[str, Any]:
    fields = [
        ...
        "defendant_claim_sum",
        "defendant_claim_max",
        "defendant_peak_cases",
        "defendant_peak_claim_sum",
        "defendant_peak_claim_max",
        "defendant_last_month_cases",
        "defendant_last_month_claim_sum",
        "defendant_cases_per_month",
        "defendant_claim_per_month",
        "defendant_avg_claim_per_case",
        "defendant_peak_claim_share",
        "plaintiff_claim_sum",
        "plaintiff_claim_max",
        "plaintiff_peak_cases",
        "plaintiff_peak_claim_sum",
        "plaintiff_peak_claim_max",
        "plaintiff_last_month_cases",
        "plaintiff_last_month_claim_sum",
        "plaintiff_cases_per_month",
        "plaintiff_claim_per_month",
        "plaintiff_avg_claim_per_case",
        "plaintiff_peak_claim_share",
    ]
    if burst is None:
        return (
            {f"{prefix}_{field}": (pd.NaT if field.endswith("date") else np.nan) for field in fields}
            | {f"{prefix}_has_burst": 0}
            | court_pressure_features(prefix, 0.0, 0.0)
        )
    output = {f"{prefix}_{field}": burst.get(field) for field in fields}
    output[f"{prefix}_has_burst"] = 1
    output |= court_pressure_features(prefix, output.get(f"{prefix}_defendant_claim_sum"), output.get(f"{prefix}_plaintiff_claim_sum"))
    return output
...
def historical_profile_features(
    company_inn: str,
    financial_years: dict[int, dict[str, float]],
    anchor_date: pd.Timestamp,
    registry_authorized_capital: Any = None,
) -> dict[str, Any]:
    output: dict[str, Any] = {
        "profile_capital_sum": np.nan,
        "profile_capital_sum_source_year": np.nan,
        "profile_capital_sum_source": None,
        "profile_headcount": np.nan,
        "profile_headcount_source_year": np.nan,
        "profile_headcount_source": None,
        "profile_headcount_source_release_date": None,
        "profile_msp_category": None,
        "profile_msp_included_asof": 0,
        "profile_msp_source": None,
        "profile_msp_source_release_date": None,
        "raw_licenses_count": 0.0,
    }
    if pd.isna(anchor_date):
        return output

    cutoff_year = int(pd.Timestamp(anchor_date).year) - 1
    cutoff_date = pd.Timestamp(year=cutoff_year, month=12, day=31)

    available_years = sorted(year for year in financial_years if year <= cutoff_year)
    for year in reversed(available_years):
        capital = safe_float(financial_years.get(year, {}).get("1310"))
        if capital is not None:
            output["profile_capital_sum"] = capital
            output["profile_capital_sum_source_year"] = year
            output["profile_capital_sum_source"] = "finances_1310"
            break

    payload = read_json(company_raw_dir(company_inn) / "company.json")
    data = payload.get("data") or {}
    opf = data.get("ОКОПФ") or {}
    opf_code = str(opf.get("Код") or "")
    opf_name = str(opf.get("Наим") or "").lower()

    if pd.isna(output["profile_capital_sum"]):
        registry_capital = safe_float((data.get("УстКап") or {}).get("Сумма"))
        if registry_capital is None:
            registry_capital = safe_float(registry_authorized_capital)
        if registry_capital is not None:
            output["profile_capital_sum"] = registry_capital
            output["profile_capital_sum_source"] = "registry_authorized_capital"

    if pd.isna(output["profile_capital_sum"]) and is_no_authorized_capital_form(opf_code, opf_name):
        output["profile_capital_sum"] = 0.0
        output["profile_capital_sum_source"] = "not_applicable_legal_form"

    rsmp = fns_rsmp_asof(company_inn, pd.Timestamp(anchor_date))
    if rsmp is not None:
        if pd.notna(rsmp.get("profile_headcount")):
            output["profile_headcount"] = safe_float(rsmp.get("profile_headcount"))
            output["profile_headcount_source_year"] = safe_int(rsmp.get("profile_headcount_source_year"))
            output["profile_headcount_source"] = "fns_rsmp_open_data"
            output["profile_headcount_source_release_date"] = pd.Timestamp(rsmp["rsmp_release_date"]).date().isoformat()
        if pd.notna(rsmp.get("profile_msp_category")):
            output["profile_msp_category"] = rsmp.get("profile_msp_category")
            output["profile_msp_included_asof"] = 1
            output["profile_msp_source"] = "fns_rsmp_open_data"
            output["profile_msp_source_release_date"] = pd.Timestamp(rsmp["rsmp_release_date"]).date().isoformat()

    license_count = 0
    for license_item in data.get("Лиценз") or []:
        start = safe_date(license_item.get("ДатаНач") or license_item.get("Дата"))
        end = safe_date(
            license_item.get("ДатаОконч")
            or license_item.get("ДатаПрекр")
            or license_item.get("ДатаАннул")
            or license_item.get("ДатаПриост")
        )
        if pd.notna(start) and start <= cutoff_date and (pd.isna(end) or end >= cutoff_date):
            license_count += 1
    output["raw_licenses_count"] = float(license_count)
    return output

def is_no_authorized_capital_form(opf_code: str, opf_name: str) -> bool:
    if opf_code.startswith(("65", "70", "75")):
        return True
    no_capital_markers = (
        "учрежден",
        "фонд",
    ...
    return any(marker in opf_name for marker in no_capital_markers)
...
def assessment_date_for_training(outcome_date: pd.Timestamp | Any) -> pd.Timestamp:
    if pd.notna(outcome_date):
        return pd.Timestamp(outcome_date) - pd.Timedelta(days=1)
    return OBSERVATION_END - pd.DateOffset(months=FORECAST_HORIZON_MONTHS)
...
def build_rows() -> pd.DataFrame:
    ...
    skipped_before_min_outcome = 0
    skipped_after_max_outcome = 0
    skipped_without_defendant_stress = 0
    for index, sample_row in enumerate(sample.to_dict("records"), start=1):
        ...
        if sample_row.get("final_status_label") == "failed" and pd.isna(outcome_date):
            skipped_missing_outcome += 1
            if skipped_missing_outcome <= 5 or skipped_missing_outcome % 25 == 0:
                print(
                    f"[build-dataset] skip failed without outcome date "
                    f"{skipped_missing_outcome}: INN={company_inn}",
                    flush=True,
                )
            continue
        if pd.notna(outcome_date) and MIN_FAILED_OUTCOME_DATE is not None and outcome_date < MIN_FAILED_OUTCOME_DATE:
            skipped_before_min_outcome += 1
            if skipped_before_min_outcome <= 5 or skipped_before_min_outcome % 25 == 0:
                print(
                    f"[build-dataset] skip outcome before min date "
                    f"{skipped_before_min_outcome}: INN={company_inn} "
                    f"outcome_date={pd.Timestamp(outcome_date).date()} min={MIN_FAILED_OUTCOME_DATE.date()}",
                    flush=True,
                )
            continue
        if pd.notna(outcome_date) and MAX_FAILED_OUTCOME_DATE is not None and outcome_date > MAX_FAILED_OUTCOME_DATE:
            skipped_after_max_outcome += 1
            if skipped_after_max_outcome <= 5 or skipped_after_max_outcome % 25 == 0:
                print(
                    f"[build-dataset] skip outcome after max date "
                    f"{skipped_after_max_outcome}: INN={company_inn} "
                    f"outcome_date={pd.Timestamp(outcome_date).date()} max={MAX_FAILED_OUTCOME_DATE.date()}",
                    flush=True,
                )
            continue
        assessment_date = assessment_date_for_training(outcome_date)
        lookback_start = assessment_date - pd.DateOffset(years=COURT_LOOKBACK_YEARS)

        company_profile = company_profile_from_raw(company_inn)
        raw_profile = raw_profile_features(company_inn)
        case_frame = build_case_frame(company_inn, lookback_start, assessment_date)
        ...
        case_pre_cutoff = case_frame[
            (case_frame["case_date"] <= assessment_date) & (case_frame["case_date"] >= lookback_start)
        ].copy()
        if REQUIRE_DEFENDANT_STRESS and int(case_pre_cutoff["is_defendant"].sum()) == 0:
            skipped_without_defendant_stress += 1
            if skipped_without_defendant_stress <= 5 or skipped_without_defendant_stress % 25 == 0:
                print(
                    f"[build-dataset] skip without defendant stress "
                    f"{skipped_without_defendant_stress}: INN={company_inn}",
                    flush=True,
                )
            continue
        ...
        row: dict[str, Any] = {
            ...
            **raw_profile,
            **historical_profile_features(company_inn, finances, assessment_date, sample_row.get("authorized_capital")),
            **aggregate_case_window(case_pre_cutoff, assessment_date, 12, "final_window12"),
            **aggregate_case_window(case_pre_cutoff, assessment_date, 18, "final_window18"),
            **aggregate_case_window(case_pre_cutoff, assessment_date, 24, "final_window24"),
            **aggregate_previous_case_window(case_pre_cutoff, assessment_date, 12, "final_prev12"),
            **aggregate_case_window(case_pre_cutoff, assessment_date, 999, "final_alltime"),
            **snapshot_features(finances, assessment_date, "final_fin"),
            **enforcement_features(enforcement_frame, assessment_date, "final_enf"),
            **efrsb_features(efrsb_frame, assessment_date, "final_efrsb"),
            ...
            **burst_features(maxsafe_burst, "maxsafe"),
            ...
            "maxsafe_to_last_defendant_claim_log_signed": signed_log_ratio(
                maxsafe_defendant_claim,
                last_defendant_claim,
                CLAIM_RATIO_MONEY_FLOOR,
            ),
            "maxsafe_to_last_defendant_claim_signed_ratio": signed_fold_ratio(
                maxsafe_defendant_claim,
                last_defendant_claim,
                CLAIM_RATIO_MONEY_FLOOR,
            ),
            "failed_within_12m_from_assessment": burst_target(outcome_date, assessment_date, 12),
            "failed_within_18m_from_assessment": burst_target(outcome_date, assessment_date, 18),
        }
        if pd.isna(row.get("region_code")) or str(row.get("region_code")).strip() == "":
            row["region_code"] = row.get("profile_region_code")
        ...
        for name, burst in (
            ...
            if name == "last":
                ...
                feature_anchor_date = assessment_date
            else:
                window_prefix = "maxsafe"
                feature_anchor_date = anchor_date
            ...
            feature_case_frame = case_pre_cutoff[case_pre_cutoff["case_date"] <= feature_anchor_date]
            row |= snapshot_features(finances, feature_anchor_date, f"{name}_anchor_fin")
            row |= enforcement_features(enforcement_frame, feature_anchor_date, f"{name}_anchor_enf")
            row |= efrsb_features(efrsb_frame, feature_anchor_date, f"{name}_anchor_efrsb")
            row |= aggregate_case_window(feature_case_frame, feature_anchor_date, 12, f"{window_prefix}_window12")
            row |= aggregate_case_window(feature_case_frame, feature_anchor_date, 18, f"{window_prefix}_window18")
            row |= aggregate_case_window(feature_case_frame, feature_anchor_date, 24, f"{window_prefix}_window24")
            row |= aggregate_previous_case_window(feature_case_frame, feature_anchor_date, 12, f"{window_prefix}_prev12")
        ...
        row["anchor_variant"] = "assessment_date"
        row["failed_within_12m_from_anchor"] = row["failed_within_12m_from_assessment"]

        rows.append(row)
        elapsed = time.perf_counter() - started_at
        if index == 1 or index % 25 == 0 or elapsed > 5:
            print(f"[build-dataset] {index}/{total} INN={company_inn} elapsed={elapsed:.2f}s", flush=True)

    frame = pd.DataFrame(rows)
    if skipped_missing_outcome:
        print(f"[build-dataset] skipped_failed_without_outcome_date={skipped_missing_outcome}", flush=True)
    if skipped_before_min_outcome:
        print(f"[build-dataset] skipped_outcome_before_min_date={skipped_before_min_outcome}", flush=True)
    if skipped_after_max_outcome:
        print(f"[build-dataset] skipped_outcome_after_max_date={skipped_after_max_outcome}", flush=True)
    if skipped_without_defendant_stress:
        print(f"[build-dataset] skipped_without_defendant_stress={skipped_without_defendant_stress}", flush=True)
    return frame

def main() -> None:
    global BASE_SEED_FILE, RAW_RUN_DIR, OUTPUT_FILE, OBSERVATION_END, REQUIRE_DEFENDANT_STRESS, BURST_MAX_EMPTY_MONTHS
    global MIN_FAILED_OUTCOME_DATE, MAX_FAILED_OUTCOME_DATE
    parser = argparse.ArgumentParser(description="Build model dataset from collected raw Ofdata payloads.")
    parser.add_argument("--seed-file", default=str(BASE_SEED_FILE))
    parser.add_argument("--raw-dir", default=str(RAW_RUN_DIR))
    parser.add_argument("--output-file", default=str(OUTPUT_FILE))
    parser.add_argument(
        "--allow-no-defendant-stress",
        action="store_true",
        help="Keep companies with no defendant-side legal cases in the assessment lookback window.",
    )
    parser.add_argument(
        "--burst-max-empty-months",
        type=int,
        default=BURST_MAX_EMPTY_MONTHS,
        help="How many empty months may be inside one court burst. Default 1 preserves old behavior; 0 requires consecutive active months.",
    )
    parser.add_argument(
        "--observation-end",
        default=OBSERVATION_END.date().isoformat(),
        help="Observation cutoff used for survived rows. Default keeps the original training cutoff.",
    )
    parser.add_argument(
        "--min-failed-outcome-date",
        default=MIN_FAILED_OUTCOME_DATE.date().isoformat(),
        help="Drop rows with a negative outcome date before this date. Pass empty string to disable.",
    )
    parser.add_argument(
        "--max-failed-outcome-date",
        default=MAX_FAILED_OUTCOME_DATE.date().isoformat(),
        help="Drop rows with a negative outcome date after this date. Pass empty string to disable.",
    )
    args = parser.parse_args()

    BASE_SEED_FILE = Path(args.seed_file)
    RAW_RUN_DIR = Path(args.raw_dir)
    OUTPUT_FILE = Path(args.output_file)
    OBSERVATION_END = pd.Timestamp(args.observation_end)
    REQUIRE_DEFENDANT_STRESS = not args.allow_no_defendant_stress
    BURST_MAX_EMPTY_MONTHS = max(args.burst_max_empty_months, 0)
    MIN_FAILED_OUTCOME_DATE = parse_optional_timestamp(args.min_failed_outcome_date)
    MAX_FAILED_OUTCOME_DATE = parse_optional_timestamp(args.max_failed_outcome_date)
