...
DEFAULT_COURT_LOOKBACK_YEARS = 2
...
TARGET_RU = {
    ...
    "failed_within_12m_from_assessment": "негативный исход в течение 12 месяцев от даты оценки",
}
...
def collect_pages(
    ...
    *,
    ...
    try:
        ...
        if output_file.exists() and not force:
            return (read_json(output_file).get("pages") or [])
        raise
    write_json(
        output_file,
        {
...
def load_company_inputs(
    ...
    *,
    ...
    try:
        ...
        court_date_from = OBSERVATION_END - pd.DateOffset(years=court_lookback_years)
        case_frame = bem.build_case_frame(inn, court_date_from, OBSERVATION_END)
    ...
    try:
        ...
        try:
            row = {
                ...
                **bem.historical_profile_features(inn, finances, prediction_date, sample_row.get("authorized_capital")),
                ...
                **bem.enforcement_features(enforcement_frame, maxsafe_anchor_date, "maxsafe_anchor_enf"),
                ...
                **bem.aggregate_case_window(
                ...
                "maxsafe_to_last_defendant_claim_log_signed": bem.signed_log_ratio(
                    maxsafe_defendant_claim,
                    last_defendant_claim,
                    bem.CLAIM_RATIO_MONEY_FLOOR,
                ),
                "maxsafe_to_last_defendant_claim_signed_ratio": bem.signed_fold_ratio(
                    maxsafe_defendant_claim,
                    last_defendant_claim,
                    bem.CLAIM_RATIO_MONEY_FLOOR,
                ),
            ...
            row |= defendant_count24_replacements(row)
        finally:
            bem.RAW_RUN_DIR = original_raw_dir
        return row
    except KeyError:
...
def defendant_count24_replacements(row: dict[str, Any]) -> dict[str, Any]:
    value = bem.safe_float(row.get(SOURCE_DEFENDANT_COUNT24)) or 0.0
    bucket = pd.cut(
        pd.Series([value]),
    ...
    return {
        DEFENDANT_COUNT24_BUCKET: None if pd.isna(bucket) else str(bucket),
    }
...
def court_history_context(inn: str, company_dir: Path, *, court_lookback_years: int = DEFAULT_COURT_LOOKBACK_YEARS) -> dict[str, Any]:
    ...
    total_cases = int(case_frame["case_uuid"].nunique()) if not case_frame.empty else 0
    defendant_frame = case_frame[case_frame["is_defendant"].astype(bool)].copy() if not case_frame.empty else case_frame
    plaintiff_cases = int(case_frame.loc[case_frame["is_plaintiff"].astype(bool), "case_uuid"].nunique()) if not case_frame.empty else 0

    if case_frame.empty:
        return {
            "total_cases_fetched": 0,
            "defendant_cases_fetched": 0,
            "plaintiff_cases_fetched": 0,
    ...
    if defendant_frame.empty:
        return {
            "total_cases_fetched": total_cases,
            "defendant_cases_fetched": 0,
            "plaintiff_cases_fetched": plaintiff_cases,
            "first_case_date": None,
            "last_case_date": None,
            "active_months": 0,
            "span_months": 0,
    ...
    monthly = (
        defendant_frame.groupby("case_month", as_index=False)
    ...
    return {
        "total_cases_fetched": total_cases,
        "defendant_cases_fetched": int(defendant_frame["case_uuid"].nunique()),
        "plaintiff_cases_fetched": plaintiff_cases,
        "first_case_date": str(defendant_frame["case_date"].min().date()),
        "last_case_date": str(defendant_frame["case_date"].max().date()),
...
def industry_baseline(row: dict[str, Any]) -> dict[str, Any]:
    ...
    cases = bem.safe_float(row.get("last_burst_defendant_cases")) or 0.0
    claim = bem.safe_float(row.get("last_burst_defendant_claim_sum")) or 0.0
    case_p90 = pd.to_numeric(subset["last_burst_defendant_cases"], errors="coerce").quantile(0.90)
    claim_p90 = pd.to_numeric(subset["last_burst_defendant_claim_sum"], errors="coerce").quantile(0.90)
    return {
    ...
    *,
    ...
    sources = {
        ...
        "finances": {
            ...
            "note": "Финансовая отчетность: исторический масштаб используется в ML-score, баланс и выручка выводятся как контекст",
        },
        "enforcements": {
...
def build_timeline(
    ...
    *,
    ...
    if not case_frame.empty:
        temp = case_frame.copy()
        temp["defendant_claim_amount"] = np.where(temp["is_defendant"].astype(bool), temp["claim_amount"], 0.0)
        case_monthly = (
            temp.groupby("case_month", as_index=False)
            .agg(
    ...
    for month in months:
        ...
        row = {
            ...
            "defendant_claim_sum": float(case_row["defendant_claim_sum"].iloc[0]) if not case_row.empty else 0.0,
...
def build_executive_summary(
    ...
    if has_burst(burst):
        summary.append(
            f"Последний судебный всплеск длился {fmt_value(burst.get('months'))} {plural(burst.get('months'), ('месяц', 'месяца', 'месяцев'))}, включал {fmt_value(burst.get('defendant_cases'))} {plural(burst.get('defendant_cases'), ('дело', 'дела', 'дел'))} к компании как к ответчику и {fmt_money(burst.get('defendant_claim_sum'))} требований."
        )
    else:
...
def model_input_columns(model: Any, fallback_columns: list[str]) -> list[str]:
    calibrated = getattr(model, "calibrated_classifiers_", None)
    if calibrated:
        estimator = getattr(calibrated[0], "estimator", None)
        preprocessor = getattr(estimator, "named_steps", {}).get("preprocessor") if estimator is not None else None
...
def score_predictive(row: dict[str, Any], service_columns: list[str]) -> float:
    ...
    model_columns = model_input_columns(model, service_columns)
    frame = pd.DataFrame([row])
    missing = {column: np.nan for column in model_columns if column not in frame.columns}
    if missing:
        frame = pd.concat([frame, pd.DataFrame([missing])], axis=1)
    return float(model.predict_proba(frame[model_columns])[:, 1][0])
...
def has_burst(burst: dict[str, Any]) -> bool:
    if not isinstance(burst, dict):
        return False
    case_field = "defendant_cases" if "defendant_cases" in burst else "total_cases"
    total_cases = bem.safe_float(burst.get(case_field)) or 0.0
...
def has_court_history(court_history: dict[str, Any]) -> bool:
    if "defendant_cases_fetched" in court_history:
        return int(court_history.get("defendant_cases_fetched") or 0) > 0
...
def short_summary(assessment: dict[str, Any], report_path: Path) -> str:
    ...
    if has_burst(burst):
        burst_line = f"Последний судебный эпизод: {fmt_date(burst.get('end_date'))} | ответчик-дел: {fmt_value(burst.get('defendant_cases'))} | сумма к компании: {fmt_money(burst.get('defendant_claim_sum'))}"
    else:
...
def build_report(assessment: dict[str, Any]) -> str:
    ...
    if has_burst(burst):
        lines.extend(
            [
                ...
                f"- Судебных дел к компании как к ответчику: `{fmt_value(burst.get('defendant_cases'))} {plural(burst.get('defendant_cases'), ('дело', 'дела', 'дел'))}`",
                f"- Сумма требований к компании как к ответчику: {fmt_money(burst.get('defendant_claim_sum'))}",
                f"- Ответчик-дел в месяц: `{fmt_value(burst.get('defendant_cases_per_month'))}`",
                f"- Сумма требований к компании в месяц: {fmt_money(burst.get('defendant_claim_per_month'))}",
    ...
    if has_court_history(court_history):
        ...
        lines.extend(
            [
                f"- Найдено судебных дел всего: `{fmt_value(court_history.get('total_cases_fetched'))}`",
                f"- Из них компания была ответчиком: `{fmt_value(court_history.get('defendant_cases_fetched'))}`",
                *(
                    [
                ...
                f"- Последние 12 месяцев: `{fmt_value(last12.get('defendant_count'))}` ответчик-дел, `{fmt_money(last12.get('defendant_claim_sum'))}` требований.",
                f"- Предыдущие 12 месяцев: `{fmt_value(previous12.get('defendant_count'))}` ответчик-дел, `{fmt_money(previous12.get('defendant_claim_sum'))}` требований.",
                f"- Все 24 месяца: `{fmt_value(last24.get('defendant_count'))}` ответчик-дел, `{fmt_money(last24.get('defendant_claim_sum'))}` требований.",
                f"- Изменение последнего года к предыдущему: `{fmt_value((change.get('defendant_count') or {}).get('diff'))}` ответчик-дел, `{fmt_money((change.get('defendant_claim_sum') or {}).get('diff'))}` требований.",
    ...
    if timeline.get("nonzero_rows"):
        lines.extend(
            [
                ...
                "",
                "| Месяц | Во всплеске | Ответчик | Истец | Сумма требований к компании | ФССП | ЕФРСБ |",
                "|---|---:|---:|---:|---:|---:|---:|",
            ]
        )
        for item in timeline.get("nonzero_rows") or []:
            lines.append(
                f"| {item.get('month')} | {'да' if item.get('in_burst') else 'нет'} | {item.get('defendant_cases')} | {item.get('plaintiff_cases')} | {fmt_money(item.get('defendant_claim_sum'))} | {item.get('enforcement_count')} | {item.get('efrsb_count')} |"
            )
    else:
        lines.extend(
            [
    ...
    lines.extend(
        [
            ...
            "- Счетчик лицензий не используется в текущем ML-score: признак исключен после ablation-проверки как потенциальный profile proxy.",
            *(
                [
...
def assess_company(
    ...
    *,
    ...
    score = {
        ...
        "target": service_metrics.get("target") or "failed_within_12m_from_assessment",
    ...
    last_burst = {
        ...
        "defendant_claim_sum": row.get("last_burst_defendant_claim_sum"),
        "defendant_cases_per_month": row.get("last_burst_defendant_cases_per_month"),
        "defendant_claim_per_month": row.get("last_burst_defendant_claim_per_month"),
        "plaintiff_cases": row.get("last_burst_plaintiff_cases"),
        "plaintiff_claim_sum": row.get("last_burst_plaintiff_claim_sum"),
    }
    ...
    context = {
        ...
        "profile_msp_category": row.get("profile_msp_category"),
        "licenses_count": bem.safe_float(row.get("raw_licenses_count")),
        "licenses_used_in_model": "raw_licenses_count" in load_service_columns(),
